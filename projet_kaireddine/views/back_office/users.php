...
try {
    $sql = "SELECT id, fullname, email, password, role, age, address, bio, interests, COALESCE(is_banned, 0) as is_banned, created_at FROM users WHERE 1=1";
    $params = [];
    ...
    $stmt = $pdo->query("SELECT COUNT(*) as total FROM users WHERE COALESCE(is_banned, 0) = 1");
    $totalBannedUsers = $stmt->fetch(PDO::FETCH_ASSOC)['total'];
                    ...
                    <table>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <span class="badge <?php echo ($user['is_banned'] == 1) ? 'banned' : 'active'; ?>">
                                        <?php echo ($user['is_banned'] == 1) ? 'Banni' : 'Actif'; ?>
                                    </span>
                                </td>
                                <td>
                                ...
                                <td>
                                    <div class="action-buttons">
                                        ...
                                        <a 
                                            href="../../controllers/userControllers.php?action=toggle_ban&id=<?php echo $user['id']; ?>" 
                                            class="btn <?php echo ($user['is_banned'] == 1) ? 'btn-success' : 'btn-warning'; ?> btn-sm"
                                            title="<?php echo ($user['is_banned'] == 1) ? 'Débannir' : 'Bannir'; ?>"
                                            onclick="return confirm('Voulez-vous vraiment <?php echo ($user['is_banned'] == 1) ? 'débannir' : 'bannir'; ?> cet utilisateur?');"
                                        >
                                            <i class="fas <?php echo ($user['is_banned'] == 1) ? 'fa-unlock' : 'fa-ban'; ?>"></i>
                                        </a>
