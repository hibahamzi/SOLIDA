...
require_once __DIR__ . '/../../Config.php'; // Inclure Config.php en premier
    ...
    if ($validation_ok) {
        try {
            ...
            if ($success) {
                ...
                $message_soumission = '<div class="error-message">❌ Erreur lors de l\'enregistrement de la réponse. Veuillez vérifier les logs du serveur pour plus de détails.</div>';
            }
        } catch (Exception $e) {
...
<head>
    ...
    <title>Modifier votre Réponse  #<?php echo htmlspecialchars($id); ?></title>
    <style>
        :root { --color-primary: #4CAF50; --color-light: #E8F5E9; --color-dark: #388E3C; --color-text: #333; --color-error: #F44336; }
        body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background: linear-gradient(135deg, #f5f7fa 0%, #c3cfe2 100%); padding: 20px; display: flex; justify-content: center; align-items: center; min-height: 100vh; margin: 0; }
        .form-container { width: 100%; max-width: 1000px; background: white; padding: 40px; border-radius: 12px; box-shadow: 0 15px 35px rgba(50, 50, 93, 0.1), 0 5px 15px rgba(0, 0, 0, 0.07); border-top: 5px solid var(--color-primary); }
...
<div class="form-container">
    <h2><span style="font-size: 32px;">📝</span> Modifier la Réponse à la Réclamation #<?php echo htmlspecialchars($id); ?></h2>
    ...
    <form method="POST" id="responseForm">
        ...
        <button type="submit" class="submit-btn" id="submitBtn">
            ...
            <span>Envoyer la modification du votre réponse par email</span>
        </button>
